...
df_avg_2024 <- subset(df_avg, ((df$TIMESTAMP > as.POSIXct("2024-01-01 00:00")) & (df$TIMESTAMP < as.POSIXct("2024-09-16 00:00"))))
...
tempplotch4 <- ggplot(data = df_avg_2023) +
  ...
  geom_point(aes(x = day, y = FCH4_F*60*60*24*(1/1000000000)*12, colour = "Methane Flux"))+
  scale_x_datetime(name = expression(""))+
  scale_y_continuous(
...
seasonal_integrals_full$season <- factor(seasonal_integrals_full$season, levels = c("Winter 2022/2023", "Snow Melt 2023", "Growing Season 2023", "Fall Senescence 2023", "Winter 2023/2024", "Snow Melt 2024", "Growing Season 2024"))

ggplot(seasonal_integrals_full, aes(x = season, y = integral_value)) +
  theme_bw()+
  ...
  geom_label(aes(label = integral_value), vjust = ifelse(seasonal_integrals_full$integral_value >= 0, -0.7, 1.5), colour = "black", fill ="white")+
  scale_y_continuous(limits=c(-53, 23))
...
# Average biomet by season
```{r}

df_avg <- df_23_24 %>%
  group_by(season) %>%
  summarize(
    avg_TA = mean(TA, na.rm = TRUE),
    avg_TS_05 = mean(TS_2_05_1, na.rm = TRUE),
    avg_D_SNOW = mean(D_SNOW, na.rm = TRUE),
    avg_SWC = mean(SWC_1_1_1, na.rm = TRUE)
    )
...
ggplot(data = df_avg_2023) +
  ...
  #geom_point(aes(x = day, y = FCH4_F/30), color = 'orange') +
  #geom_point(aes(x = day, y = RECO), color = 'purple') +
  geom_line(aes(x = day, y = TS_2_05_1/3), color = 'blue') + 
  geom_line(aes(x = day, y = SWC_1_1_1/30), color = 'red') +
  ...
  scale_x_datetime(limits = as.POSIXct(c('2023-02-05', '2023-06-30')))+
...
ggplot(data = df_avg_2024) +
  ...
  scale_x_datetime(limits = as.POSIXct(c('2024-01-01', '2024-06-30')))+
  scale_y_continuous(limits = c(-2.5, 6))
...
ggplot(data = df_avg_23_24) +
  ...
  geom_point(aes(x = day, y = FCH4/30), color = 'orange') +
  # geom_point(aes(x = day, y = RECO), color = 'purple') +
  geom_line(aes(x = day, y = TS_2_05_1/6), color = 'blue') + 
  geom_line(aes(x= day, y= TA*(1/20)), color="purple")+
  #geom_point(aes(x=day, y = G_1_1_1*(1/50)), color = "grey")+
  geom_line(aes(x = day, y = SWC_1_1_1/30), color = 'red') +
  geom_line(aes(x = day, y = D_SNOW/20), color = 'darkgreen') +
