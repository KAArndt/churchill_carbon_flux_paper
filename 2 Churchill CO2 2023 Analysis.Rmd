...
df = fread(input = "AMF_CA-CF3_BASE_HH_1-5.csv",na.strings = c("-9999"))
df$ts = as.POSIXct(x = as.character(df$TIMESTAMP_END),format = "%Y%m%d%H%M")
```
...
theme(axis.text.x = element_text(size = 12), axis.text.y = element_text(size = 12), 
  ...
  geom_label(aes(label = Value), hjust = ifelse(net_wp$Value >= 0, -0.7, 1.5), colour = "black", fill ="white", size = 5)+
...
df_avg_23_24 <- df_avg_23_24 %>%
  mutate(
      ...
      (date >= as.POSIXct("2022-10-16") & date <= as.POSIXct("2023-05-04")) ~ 'Winter 2022',
      ...
      (date >= as.POSIXct("2023-10-22") & date <= as.POSIXct("2024-05-14")) ~ 'Winter 2023',
...
theme(axis.text.x = element_text(size = 12), axis.text.y = element_text(size = 12), 
  ...
  annotate("text", x = as.POSIXct("2024-07-30"), y = 1.7, label = "Growing\nSeason", size = 5)+
  annotate("text", x = as.POSIXct("2023-07-26"), y = 1.7, label = "Growing\nSeason", size = 5)+
  annotate("text", x = as.POSIXct("2024-05-31"), y = 1.7, label = "Snow\nMelt", size = 5)+
  annotate("text", x = as.POSIXct("2023-05-25"), y = 1.7, label = "Snow\nMelt", size = 5)+
  annotate("text", x = as.POSIXct("2023-09-30"), y = 1.7, label = "Fall", size = 5)+
  annotate("text", x = as.POSIXct("2022-09-25"), y = 1.7, label = "Fall", size = 5)+
  annotate("text", x = as.POSIXct("2024-09-30"), y = 1.7, label = "Fall", size = 5)+
  annotate("text", x = as.POSIXct("2024-02-05"), y = 1.7, label = "Winter", size = 5)+
  annotate("text", x = as.POSIXct("2023-01-30"), y = 1.7, label = "Winter", size = 5)
...
seasonal_integrals_full$season <- factor(seasonal_integrals_full$season, levels = c("Fall Senescence 2022", "Winter 2022", "Snow Melt 2023", "Growing Season 2023", "Fall Senescence 2023", "Winter 2023", "Snow Melt 2024", "Growing Season 2024", "Fall Senescence 2024"))

#Filter to remove incomplete fall seasons
filtered_data <- seasonal_integrals_full %>%
  filter(season %in% c("Winter 2022", "Snow Melt 2023", "Growing Season 2023", "Fall Senescence 2023", "Winter 2023", "Snow Melt 2024", "Growing Season 2024"))
...
## Table of average values with standard error, averaged over seasons across both years
df_std_error <- df_23_24 %>%
  group_by(season) %>%
  summarize(
    nee = round(mean(FC, na.rm = TRUE), 2),
    tair = round(mean(TA, na.rm = TRUE), 2),
    tsoil5 = round(mean(TS_2_05_1, na.rm = TRUE), 2),
    snow = round(mean(D_SNOW, na.rm = TRUE), 2),
    ...
    snow.se = round(std.error(D_SNOW, na.rm = TRUE), 2)
    ) %>%
  mutate(
    nee_with_se = paste0(nee, " ± ", nee.se),  # Combine value and SE
    tair_with_se = paste0(tair, " ± ", tair.se),
    tsoil5_with_se = paste0(tsoil5, " ± ", tsoil5.se),
    snow_with_se = paste0(snow, " ± ", snow.se)
  ) %>%
  select(season, nee_with_se, tair_with_se, tsoil5_with_se, snow_with_se)



gt(df_std_error, rowname_col = "season_name") %>% 
  tab_header(title = "Seasonal Averages",
             subtitle = "Sept. 15th, 2022 to Sept 15th, 2024") %>%
  cols_label(
    ...
    nee_with_se = md("NEE<br><span style='font-size:10pt'>(&micro;molCO<sup>2</sup>m<sup>-2</sup>s<sup>-1</sup>)</span>")
  )%>%
  # cols_width(
  #   stub() ~ px(200)
  # )%>%
...
ch4_23 <- ggplot(data = df_avg_23_24) +
  ...
  ggtitle("Spring 2023")+
  scale_y_continuous(
    name = expression("Methane Flux (g CH"[4]*"-C"~m^-2~h^-1~")"),
...
ch4_24 <- ggplot(data = df_avg_23_24) +
  ...
  scale_color_manual(name = " ",
            values = c("Methane Flux" = "turquoise",
                      "5cm Soil Temperature" = "navy",
                      "Air Temperature" = "salmon"),
            breaks = c("Methane Flux", "5cm Soil Temperature", "Air Temperature"))
ch4_24
...
co2_23 <- ggplot(data = df_avg_23_24) +
  ...
  ggtitle("Spring 2023")+
  scale_y_continuous(
    name = expression("Nighttime NEE (g CO"[2]*"-C"~m^-2~h^-1~")"),
    limits = c(-0.1, 0.1),
...
soiltemp <- ggplot()+
  theme_bw()+
  geom_pointrange(data = df_soiltemp, aes(x= Time, y= -Depth, ymin = -100, ymax = 0))+
...
df_merge <- merge(df_avg_2023, df_avg_2024, by = "DOY", suffixes = c(".23", ".24"))

#2023
#co2
exp_model.23 <- nls(FC_night.23 ~ a * exp(b * TS_1_05_1.23), data = df_merge, start = list(a = 1, b = 0.1))

# Generate predicted values
df_merge$predicted.23 <- predict(exp_model.23, newdata = df_merge)



#ch4
exp_model_ch4.23 <- nls(FCH4.23 ~ a * exp(b * TS_1_05_1.23), data = df_merge, start = list(a = 1, b = 0.1))

# Generate predicted values
df_merge$predicted_ch4.23 <- predict(exp_model_ch4.23, newdata = df_merge)



#2024
exp_model.24 <- nls(FC_night.24 ~ a * exp(b * TS_1_05_1.24), data = df_merge, start = list(a = 1, b = 0.1))

# Generate predicted values
df_merge$predicted.24 <- predict(exp_model.24, newdata = df_merge)

#ch4
exp_model_ch4.24 <- nls(FCH4.24 ~ a * exp(b * TS_1_05_1.24), data = df_merge, start = list(a = 1, b = 0.1))

# Generate predicted values
df_merge$predicted_ch4.24 <- predict(exp_model_ch4.24, newdata = df_merge)
...
ggplot(data=df_merge)+
  ...
  geom_point(aes(x=TS_1_05_1.23, y=FC_night.23, col = DOY, shape = "2023"))+
  geom_point(aes(x=TS_1_05_1.24, y=FC_night.24, col = DOY, shape = "2024"))+
  geom_line(aes(x = TS_1_05_1.23, y = predicted.23, linetype = "2023"))+
  geom_line(aes(x = TS_1_05_1.24, y = predicted.24, linetype = "2024"))+
scale_color_gradient(low = "navy", high = "salmon")+
  scale_shape_manual(values = c("2023" = 16, "2024" = 17))+
  scale_linetype_manual(values = c("2023" = "solid", "2024" = "dashed"))+
  labs(
...
ggplot(data=df_merge)+
  geom_vline(xintercept=0)+
  theme_bw()+
  geom_point(aes(x=TS_1_05_1.23, y=FCH4.23*(1/1000), col = DOY, shape = "2023"))+
  geom_point(aes(x=TS_1_05_1.24, y=FCH4.24*(1/1000), col = DOY, shape = "2024"))+
  geom_line(aes(x = TS_1_05_1.23, y = predicted_ch4.23*(1/1000), linetype = "2023"))+
  geom_line(aes(x = TS_1_05_1.24, y = predicted_ch4.24*(1/1000), linetype = "2024"))+
scale_color_gradient(low = "navy", high = "salmon")+
scale_linetype_manual(values = c("2023" = "solid", "2024" = "dashed"))+
  labs(
    x = expression("5cm Soil Temperature ("*degree*"C)"),
