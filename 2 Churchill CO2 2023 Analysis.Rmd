...
# png(filename = './year2.png',width = 6.25,height = 4,units = 'in',res = 2000)
# year2
# dev.off()
...
theme(axis.text.x = element_text(size = 14), axis.text.y = element_text(size = 14), 
  ...
  annotate("text", x = as.POSIXct("2022-09-25"), y = 1.55, label = "Fall", size = 3.8)+
...
#averages yearly soil temps
df_avg_soil <- df_23_24 %>%
  group_by(year) %>%
  summarize(
    tsoil5 = round(mean(TS_2_05_1, na.rm = TRUE), 2),
  tsoil5.se = round(std.error(TS_2_05_1, na.rm = TRUE), 2)
...
df_std_error <- df_23_24 %>%
  ...
  select(season, nee_with_se, tair_with_se, tsoil5_with_se)
...
gt(df_std_error, rowname_col = "season_name") %>% 
  ...
  cols_label(
    tair_with_se = md("Air Temp<br><span style='font-size:10pt'>(&deg;C)</span>"),
    tsoil5_with_se = md("5cm Soil Temp<br><span style='font-size:10pt'>(&deg;C)</span>"),
    nee_with_se = md("Daily NEE<br><span style='font-size:10pt'>(&micro;molCO<sub>2</sub>m<sup>-2</sup>s<sup>-1</sup>)</span>"),
    season = md("Season")
  )%>%
  # cols_width(
  #   stub() ~ px(200)
...
ggplot(data = df_avg_23_24) +
  ...
  axis.title = element_text(size = 6),legend.position = 'none')+
  ...
  geom_line(aes(x= day, y= SWC_2_2_1/50000, color="Variable"))+
  geom_hline(yintercept = 0) +
  #ggtitle("Spring 2023")+
  scale_y_continuous(
...
df_avg_winter <- df_avg_23_24[df_avg_23_24$season %in% c("Winter 2023", "Winter 2022"), ]
...
ggplot(data = df_avg_23_24)+ theme_bw()+
  geom_hline(yintercept = 0)+
  geom_line(aes(day,TS_2_0_1,color='Surface'))+
  ...
  geom_line(aes(day,TS_2_10_1,color='1 m'))+
