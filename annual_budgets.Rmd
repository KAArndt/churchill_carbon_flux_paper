...
budget <- ggplot(net_wp, aes(y = Category, x = Value, fill = factor(Year, levels = c("2", "1")))) +
  ...
  labs(
    ...
    fill = " ") +
  geom_label(aes(label = paste0(Value, " gC")), vjust = ifelse(net_wp$Year == 1, -1, 2), hjust = ifelse(net_wp$Value >= 0, -0.2, 1.1), colour = "black", fill ="white", size = 5)+ ## Need to fix these!
...
png(filename = './budget.png',width = 10.90,height = 6.72,units = 'in',res = 2000)
budget
dev.off()
