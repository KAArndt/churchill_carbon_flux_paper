...
# Using Kyle's error dataset here!!
df3 <- fread('./data/error_churchill_validation.csv', na.strings=c("-9999", "NA"))
...
# Ensure continuous time variable (check that his isn't doing anything - data shouldn't have gaps at all!)
...
# Remove SWC below zero

```{r}

df$SWC_1_1_1 <- ifelse(df$TS_3_1_1 < 0, NA, df$SWC_1_1_1)
df$SWC_2_1_1 <- ifelse(df$TS_4_1_1 < 0, NA, df$SWC_2_1_1)
df$SWC_2_2_1 <- ifelse(df$TS_4_1_1 < 0, NA, df$SWC_2_2_1)


```

# Compare "FC" variables and median flux values from error calculations
Trends line up well, but using the median pulls down outliars by quite a bit
...
# 1:1
ggplot(data = df)+
  geom_point(aes(FC_F, median.rf.nee))+
  geom_abline(aes(intercept= 0, slope = 1))

# Timeseries
ggplot(data = df)+
  geom_point(aes(TIMESTAMP, FC_F), color = "turquoise3")+
  geom_point(aes(TIMESTAMP, median.rf.nee), color = "coral")

## CH4
# 1:1
ggplot(data = df)+
  geom_point(aes(FCH4_F, median.rf.ch4))+
  geom_abline(aes(intercept= 0, slope = 1))

# Timeseries
ggplot(data = df)+
  geom_point(aes(TIMESTAMP, FCH4_F), color = "turquoise3")+
  geom_point(aes(TIMESTAMP, median.rf.ch4), color = "coral")


##### Reassign FC_F and FCH4_F to median values

df$FC_F_old <- df$FC_F
df$FC_F <- df$median.rf.nee

df$FCH4_F_old <- df$FCH4_F
df$FCH4_F <- df$median.rf.ch4
...
df <- df %>%
  mutate(FC_night = ifelse(SW_IN <= 20, FC, NA))

df <- df %>%
  mutate(FC_night_F = ifelse(SW_IN <= 20, FC_F, NA))
...
ggplot(data = df)+
  ...
  # scale_x_datetime(limits = as.POSIXct(c('2023-01-01','2025-12-01'),format="%F"))+
...
df_avg_sd <- timeAverage(df, avg.time = "day", data.thresh = 50, statistic = "sd")
...
df_avg_night_sd <- timeAverage(df, avg.time = "day", data.thresh = 10, statistic = "sd")
...
# add standard deviations into full dataset
df_avg$FC_night_sd = df_avg_night_sd$FC_night
df_avg$FC_night_F_sd = df_avg_night_sd$FC_night_F

df_avg$FC_sd = df_avg_sd$FC
df_avg$FC_F_sd = df_avg_sd$FC_F

df_avg$FCH4_sd = df_avg_sd$FC
df_avg$FCH4_F_sd = df_avg_sd$FC_F
...
df_avg <- select(df_avg, -c('TIMESTAMP','TIMESTAMP_END', 'TIMESTAMP_START'))
...
# }
# 
# df_avg_2025 <- year_df(df_avg, 2025)
# 
# summary(df_avg_2025)
# 
# plot(df_avg_2025$FC_night)
# 
# plot(df_avg_2025$TS_1_05_1, df_avg_2025$FC_night)
# 
# 
...
write.csv(x = df,file = './output_files/Churchill_Merged_CF1_CF3_HH_25.csv',row.names = F,quote = F)

write.csv(x = df_avg,file = './output_files/Churchill_Merged_Avg_CF1_CF3_HH_25.csv',row.names = F,quote = F)
