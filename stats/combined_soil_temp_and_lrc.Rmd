...
lrc <- ggplot(data = gs)+
  ...
  geom_point(aes(PPFD_IN,FC), color = "#FF573D", alpha = 0.3)+
  # scale_color_viridis_c(option = "magma", limits = c(1,365))+
...
co2model_ngs <- ggplot(data=df_nogs)+
  ...
  #   width = 0.2
  # )+
  geom_line(aes(x = TS_1_0_1, y = predicted), linewidth = 1.2)+
  scale_color_viridis_c(option = "magma")+
  labs(
    x = expression("Surface Soil Temperature ("*degree*"C)"),
    y = expression("ER ("*mu*mol*" CO"[2]~m^-2~s^-1*")"),
    title = expression("a) Ecosystem Respiration (ER) vs. Soil Temperature"))+
...
ch4model <- ggplot(data=df_nogs)+
  ...
  #   width = 0.2
  # )+
  geom_line(aes(x = TS_1_0_1, y = predicted_ch4), linewidth = 1.2)+
  scale_color_viridis_c(option = "magma")+
  labs(
    x = expression("Surface Soil Temperature ("*degree*"C)"),
    y = expression("CH "[4] * " Flux ("*mu*mol*" CH"[4]~m^-2~s^-1*")"),
