...
co2model_ngs <- ggplot(data=df_nogs)+
  ...
  labs(
    ...
    y = expression("Avg. Half-Hourly NEE ( "*mu*mol~m^-2~s^-1*")"),
    title = expression("Nighttime NEE vs. Soil Temperature"))+
 # scale_shape_manual(name = "Year", values = c(16, 17))+
  scale_x_continuous(limits=c(-11,25))+
  scale_y_continuous(limits=c(-1.7,2.5))+
theme(axis.text.x = element_text(size = 12), axis.text.y = element_text(size = 12), 
  ...
  annotate("text", 
           x = -Inf, y = Inf, 
           hjust = -0.1, vjust = 3,
           label = paste0("y = ", round(coef_ngs[,a], 2), "e^", round(rsq_co2_ngs, 2), "x"),
           size = 5)
...
scale_color_gradient(low = "navy", high = "salmon")+
  labs(
    x = expression("5cm Soil Temperature ("*degree*"C)"),
    y = expression("Avg. Half-Hourly CH "[4] * " Flux ("*mu*mol~m^-2~s^-1*")"),
...
Visualize model outliars
```{r}

ggplot(data = df_nogs)+
  geom_point(aes(x=date, y=FC), color = "black")+
  geom_point(aes(x=date, y=FC_night), color = "red")
