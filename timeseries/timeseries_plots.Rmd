...
Seasonal and met timeseries
```{r}

class(df_avg_23_24$season_year)
df_avg_23_24$season_year <- as.factor(df_avg_23_24$season_year)


season_ts <- ggplot(data = df_avg_23_24, aes(x=DOY))+
  theme_bw()+
  facet_wrap(~season_name, scales = "free")+
  geom_point(aes(y = FC_F*60*60*24*(1/1e6)*12, shape = "Gapfilled", color = season_year))+
  geom_point(aes(y = FC*60*60*24*(1/1e6)*12, shape = "Measured", color = season_year))+
  geom_hline(yintercept=0, col="black")+
  scale_y_continuous(limits = c(-3.2, 1.4), expression('NEE (gC-CO'[2]~m^-2~d^-1*')'))+
  # scale_x_date(name = NULL,
  #                  breaks = seq(as.Date("2022-09-01"), as.Date("2025-09-01"), by = "4 months"), date_labels = "%b %Y")+
...
swout_ts <- ggplot(data = df_avg_23_24, aes(x=date))+
  theme_bw()+
...
temp_ts <- ggplot(data = df_avg_23_24, aes(x=date))+
  ...
  geom_line(aes(y = TS_2_05_1, linetype = "5cm Soil Temperature"), color = "navy")+
  geom_line(aes(y = TA, linetype = "Air Temperature"), color = "navy")+
  geom_hline(yintercept=0, col="black")+
  scale_y_continuous(limits = c(-40, 20), expression('Temperature ('*degree*'C)'))+
  scale_x_date(name = NULL,
                   breaks = seq(as.Date("2022-09-01"), as.Date("2025-09-01"), by = "4 months"), date_labels = "%b %Y")+
  scale_linetype_manual(name = NULL,
            values = c("Air Temperature" = "solid",
                      "5cm Soil Temperature" = "dashed"),
            breaks = c("Air Temperature", "5cm Soil Temperature"))+
  scale_fill_manual(name = NULL,
            values = c("Spring Thaw" = "mediumorchid",
                      "Fall Senescence" = "orange",
                      "Winter" = "cyan3",
